...
def expansion_generation(inlines, engine, prompt_engine, output_dir,top_p):
    output_file = open(os.path.join(output_dir, '0-expansion_generation.jsonl'), 'a')
    questions = [line['question'] for line in inlines]
    batch_expansions = generate_expansion(questions, engine, prompt_engine,top_p)
...
def main():
    ...
    parser.add_argument("--prompt_dir", default='prompt_templates', type=str,
                        help="The prompt dir")
    parser.add_argument("--dataset", default='webq', type=str,
                        help="dataset name: [nq, tqa, webq]")
    parser.add_argument("--split", default='test', type=str,
                        help="dataset split: [train, dev, test]")
    # Change the engine default in the argument parser
    parser.add_argument("--engine", default='deepseek-r1-distill-llama-70b', type=str,
                        help="Groq model name: deepseek-r1-distill-llama-70b/llama2-70b-4096/mixtral-8x7b-32768")
    ...
    while index < len(inlines):
        ...
        batch_inlines = expansion_generation(batch_inlines, args.engine, prompt_engine, output_dir,args.top_p)
